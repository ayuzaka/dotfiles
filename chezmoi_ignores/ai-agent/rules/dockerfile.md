...
Avoid installing extra or unnecessary packages just because they might be nice to have. For example, you don窶冲 need to include a text editor in a database image.
...
Here窶冱 an example from the [buildpack-deps image](https://github.com/docker-library/buildpack-deps):
...
what窶冱 in the cache. This technique can also reduce failures due to unanticipated changes
...
variables specific to services you want to containerize, such as Postgres窶冱
`PGDATA`.
...
RUN curl -SL https://example.com/postgres-$PG_VERSION.tar.xz | tar -xJC /usr/src/postgres && 窶ｦ
...
> the "next" UID/GID is assigned regardless of image rebuilds. So, if it窶冱
...
like `RUN cd 窶ｦ && do-something`, which are hard to read, troubleshoot, and
maintain.
...
Dockerfile, as you can see in [Ruby窶冱 `ONBUILD` variants](https://github.com/docker-library/ruby/blob/c43fef8a60cea31eb9e7d960a076d633cb62ba8d/2.4/jessie/onbuild/Dockerfile).
