...
vim.api.nvim_create_autocmd("FileType", {
  ...
  callback = function(ctx)
    ...
    if vim.fn.findfile("package.json", ".;") ~= "" then
      local vtsls_config = vim.tbl_deep_extend("force", {}, vim.lsp.config.vtsls)
      local project_root = vim.fs.root(
        ctx.buf,
        { "package.json", "pnpm-lock.yaml", "package-lock.json", "yarn.lock", "bun.lockb", "bun.lock", ".git" }
      ) or vim.fn.getcwd()

      vtsls_config.root_dir = project_root
      vim.lsp.start(vtsls_config)
